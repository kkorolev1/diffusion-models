...
def train(rank, gpu, args):
    ...
    checkpoint_path = os.path.join(exp_path, "netG_15.pth")
    netG.load_state_dict(torch.load(checkpoint_path))
    
    print("SAMPLING")
    batch_size = 64
    num_iters = 60000//batch_size
    clf = resnet18(pretrained=True).to(device)
    clf.load_state_dict(torch.load("clf.pth"))
    p_yx = []
    
    for i in range(num_iters):
        x_t_1 = torch.randn((batch_size, 3, 32, 32)).to(device)
        fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args)
        logits = clf(fake_sample)
        tmp_p_yx = nn.functional.softmax(logits, dim=1)
        p_yx.append(tmp_p_yx)
    
    p_yx = torch.cat(p_yx)

    print("IS:", calculate_inception_score(p_yx))
    #fake_sample[:,1,...] = fake_sample[:,0,...]
    #fake_sample[:,2,...] = fake_sample[:,0,...]
    #torchvision.utils.save_image(fake_sample, os.path.join(exp_path, 'sample_discrete_epoch_{}.png'.format("X")), normalize=False)
...
def calculate_inception_score(p_yx, eps=1e-16):
    ...
    return is_score.detach().cpu().item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_content_every', type=int, default=15, help='save content for resuming every x epochs')
    parser.add_argument('--save_ckpt_every', type=int, default=15, help='save ckpt every x epochs')

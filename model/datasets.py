...
def get_MNIST(batch_size, val_ratio=0.1, train=True):
    seed = 42
...
def get_CIFAR10(batch_size, val_ratio=0.1, train=True):
    seed = 42
    torch.manual_seed(seed)
    np.random.seed(seed)
...
def get_loaders(dataset_name, batch_size, val_ratio=0.1, train=True):
    dataset_fetchers = {
        "mnist": get_MNIST,
        "cifar10": get_CIFAR10
    }
    ...
    return dataset_fetchers[dataset_name](batch_size, val_ratio, train)